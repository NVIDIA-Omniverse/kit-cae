...
from omni.kit.viewport.menubar.core import get_instance as get_mb_inst
...
ICON_PATH = carb.tokens.get_tokens_interface().resolve(
    "${% raw %}{{% endraw %}{{ extension_name }}{% raw %}}{% endraw %}/data/icons"
)
...
async def _load_layout_startup(layout_file: str, keep_windows_open: bool = False):
...
class SetupExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str):
        ...
        self._layouts_path = carb.tokens.get_tokens_interface().resolve(
            "${% raw %}{{% endraw %}{{ extension_name }}{% raw %}}{% endraw %}/layouts"
        )
        ...
        self.layout_user_path = str(Path(token.resolve("${data}")) / "layout_user.json")
    ...
    def _on_application_mode_changed(self, item: carb.dictionary.Item, _typ: carb.settings.ChangeEventType):
    ...
    def _custom_quicklayout_menu(self):
        ...
        def add_layout_menu_entry(name, parameter, key):
            ...
            layouts_path = carb.tokens.get_tokens_interface().resolve(
                "${% raw %}{{% endraw %}{{ extension_name }}{% raw %}}{% endraw %}/layouts"
            )
