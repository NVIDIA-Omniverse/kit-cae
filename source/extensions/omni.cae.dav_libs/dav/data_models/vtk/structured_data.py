...
@dav.func
def get_voxel_vertex_offset(local_idx: wp.int32) -> wp.vec3i:
    """Get the (di, dj, dk) offset for a voxel vertex in VTK ordering.

    Args:
        local_idx: Local vertex index (0-7) in VTK voxel ordering
    ...
    if local_idx == 0:  # (0, 0, 0)
        ...
        offset = wp.vec3i(1, 0, 0)
    elif local_idx == 2:  # (0, 1, 0)
        offset = wp.vec3i(0, 1, 0)
    elif local_idx == 3:  # (1, 1, 0)
        offset = wp.vec3i(1, 1, 0)
    elif local_idx == 4:  # (0, 0, 1)
        offset = wp.vec3i(0, 0, 1)
    elif local_idx == 5:  # (1, 0, 1)
        offset = wp.vec3i(1, 0, 1)
    elif local_idx == 6:  # (0, 1, 1)
        offset = wp.vec3i(0, 1, 1)
    elif local_idx == 7:  # (1, 1, 1)
        offset = wp.vec3i(1, 1, 1)
...
@dav.func
def compute_point_id_for_cell_vertex(cell_ijk: wp.vec3i, local_idx: wp.int32, extent_min: wp.vec3i, extent_dims: wp.vec3i, use_voxel_ordering: wp.bool = False) -> wp.int32:
    ...
    Args:
        ...
        extent_min: Minimum extent of the grid
        extent_dims: Dimensions (number of points) in each direction
        use_voxel_ordering: If True, use VTK voxel ordering; if False (default), use VTK hex ordering
    ...
    # Get vertex offset for this local index (VTK hexahedron or voxel ordering)
    vertex_offset = wp.vec3i(0, 0, 0)
    if use_voxel_ordering:
        vertex_offset = get_voxel_vertex_offset(local_idx)
    else:
...
class CellLinksAPI:
    ...
    @staticmethod
    @dav.func
    def get_num_cells(point_id: wp.int32, extent_min: wp.vec3i, extent_max: wp.vec3i) -> wp.int32:
        ...
        """
        if not CellLinksAPI.is_valid(point_id):
            return 0
        ...
        point_ijk = compute_point_ijk(point_id, point_dims)
        ...
        for di in range(-1, 1):
            for dj in range(-1, 1):
                for dk in range(-1, 1):
                    ...
                    if cell_ijk.x >= 0 and cell_ijk.x < cell_dims.x and cell_ijk.y >= 0 and cell_ijk.y < cell_dims.y and cell_ijk.z >= 0 and cell_ijk.z < cell_dims.z:
                        count += 1
    ...
    @staticmethod
    @dav.func
    def get_cell_id(point_id: wp.int32, local_idx: wp.int32, extent_min: wp.vec3i, extent_max: wp.vec3i) -> wp.int32:
        """Get the cell ID for a given cell index in the cell link.

        Args:
            point_id: The point ID to query
            local_idx: Local index within the cells using this point (0-based)
            extent_min: Minimum extent of the grid
            extent_max: Maximum extent of the grid
