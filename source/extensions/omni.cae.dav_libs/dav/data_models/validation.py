...
def validate_data_model(data_model: Any, verbose: bool = True) -> tuple[bool, list[str]]:
    ...
    Example:
        ...
        ...         print(error)
    """
    from dav.data_models.typing import CellAPI, CellLinksAPI, DatasetAPI
    ...
    handle_types = {"DatasetHandle": None, "CellHandle": None, "PointIdHandle": None, "CellIdHandle": None}
        ...
        if not hasattr(data_model, handle_name):
            ...
            if handle_name not in ["PointIdHandle", "CellIdHandle"]:
                ...
                if not _is_warp_type(handle_type):
                    errors.append(f"{handle_name} should be a warp type (wp.int32, wp.int64, wp.vec2i, etc.), got {type(handle_type)}")
    ...
    api_classes = {"DatasetAPI": DatasetAPI, "CellAPI": CellAPI, "CellLinksAPI": CellLinksAPI}
...
def _is_warp_type(obj: Any) -> bool:
    ...
    warp_primitives = [wp.int8, wp.int16, wp.int32, wp.int64, wp.uint8, wp.uint16, wp.uint32, wp.uint64, wp.float16, wp.float32, wp.float64, wp.bool]

    # Check for warp vector types
    warp_vectors = [wp.vec2, wp.vec3, wp.vec4, wp.vec2i, wp.vec3i, wp.vec4i, wp.vec2f, wp.vec3f, wp.vec4f, wp.vec2d, wp.vec3d, wp.vec4d]
...
def validate_all_builtin_models(verbose: bool = True) -> dict[str, tuple[bool, list[str]]]:
    ...
    try:
        from dav.data_models.vtk import unstructured_grid, vtk_shapes

        results["vtk.unstructured_grid"] = validate_data_model(unstructured_grid.get_data_model(vtk_shapes.get_supported_cell_types()), verbose=verbose)
    except ImportError:
        results["vtk.unstructured_grid"] = (False, ["Module not available"])

    # SIDS data models
    try:
        from dav.data_models.sids import sids_shapes, unstructured

        results["sids.unstructured"] = validate_data_model(unstructured.get_data_model(sids_shapes.get_supported_sids_element_types()), verbose=verbose)
    except ImportError:
    ...
    try:
        ...
        base_models = {"vtk.image_data": ("dav.data_models.vtk.image_data", "DataModel"), "vtk.structured_grid": ("dav.data_models.vtk.structured_grid", "DataModel")}
