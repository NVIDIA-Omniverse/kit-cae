...
class CellLinksAPI(Protocol):
    """Static API for operations on point ids."""

    ...

    @staticmethod
    def is_valid(point_id: PointIdHandle) -> wp.bool:
        """Check if cell link is valid for the given point id.

        Args:
            point_id: The point id to check (PointIdHandle)
        Returns:
    ...
    def get_num_cells(point_id: PointIdHandle, ds: DatasetHandle) -> wp.int32:
        """Get the number of cells for the given point id.

        Args:
            point_id: The point id to query (PointIdHandle)
            ds: The dataset containing the cell link (DatasetHandle)
        Returns:
            wp.int32: The number of cells for the given point id
        """
        ...

    @staticmethod
    def get_cell_id(point_id: PointIdHandle, cell_idx: wp.int32, ds: DatasetHandle) -> CellIdHandle:
        """Get the cell id for a given index for the given point id.
...
class CellLocatorAPI(Protocol):
    """Static API for operations on cell locators such as finding the cell containing a point or evaluating interpolation weights."""
    ...
    @staticmethod
    def point_in_cell(dataset: DatasetHandle, point: wp.vec3f, cell: CellHandle) -> wp.bool:
        """Check if a point is inside a cell.

        Args:
            dataset: The dataset to query (DatasetHandle)
            point: The point to test (wp.vec3f)
            cell: The cell to test against (CellHandle)

        Returns:
            wp.bool: True if the point is inside the cell, False otherwise
        """
        ...
...
class DataModel(Protocol):
    ...
    CellLocatorAPI: type[CellLocatorAPI]
    """Static API for cell locator operations."""
