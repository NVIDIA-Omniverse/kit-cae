...
@dav.cached
def get_field_model(field_model: dav.FieldModel) -> dav.FieldModel:
    ...
    Returns:
        dav.FieldModel: A collection field model that handles multiple field pieces
    ...
    # Standalone helper defined before CollectionFieldAPI so that get/set can call it
    # without referencing CollectionFieldAPI mid-class-body (empty closure cell).
    @dav.func
    ...
    class CollectionFieldAPI:
        ...
        @dav.func
        def _global_idx_to_piece_idx(field: CollectionFieldHandle, global_idx: wp.int32) -> wp.vec2i:
            return _global_idx_to_piece_idx(field, global_idx)
        ...
        def get(field: CollectionFieldHandle, idx: wp.int32):
            ...
            """
            piece_info = _global_idx_to_piece_idx(field, idx)
            if piece_info.x < 0:
    ...
    class CollectFieldModelMeta(type):
        def __repr__(cls):
            return f"CollectionFieldModel(base_model={field_model})"

    class CollectionFieldModel(metaclass=CollectFieldModelMeta):
