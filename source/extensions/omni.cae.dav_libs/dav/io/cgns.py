...
from dav.data_models.sids.sids_shapes import ET_MIXED, ET_NODE, ET_NFACE_n, ET_NGON_n, ET_NofValidElementTypes
...
def _extract_flow_solutions(zone_node, zone_name: str, num_points: int, device):
    ...
    for flow_sol_node in flow_solution_nodes:
        ...
        if grid_location == "Vertex":
            ...
            logger.warning("Skipping FlowSolution '%s' with unsupported GridLocation '%s'", flow_sol_name, grid_location)
            logger.warning("         DAV currently only supports Vertex and CellCenter. Face-centered and other locations are not yet implemented.")
            continue
        ...
        for node in flow_sol_node[2]:
            if node[3] == "DataArray_t":
                ...
                if len(field_data.shape) == 1:
                    ...
                    logger.info("  Loaded field '%s': %s, shape=%s (AoS vector)", full_field_name, association.name, field_data.shape)
                else:
                    logger.warning("Skipping field '%s' with unsupported shape %s", field_name, field_data.shape)
                    logger.warning("         DAV currently only supports scalar (1D) and 3-component vector (Nx3) fields.")
                ...
                if x_match:
                    ...
                    if y_name and z_name:
                        ...
                        if len(x_data) == len(y_data) == len(z_data):
                            ...
                            fields[full_field_name] = field
                            logger.info(
                                "  Loaded field '%s': %s, shape=(%d, 3) (SoA vector from %s, %s, %s)", full_field_name, association.name, len(x_data), field_name, y_name, z_name
                            )
                            ...
                            logger.warning("Vector components %s, %s, %s have mismatched lengths", field_name, y_name, z_name)
        ...
        for field_name, field_data in scalar_fields.items():
            ...
            fields[full_field_name] = field
            logger.info("  Loaded field '%s': %s, shape=%s (scalar)", full_field_name, association.name, field_data.shape)

    return fields


def _create_nface_n_dataset(section_name, nface_connectivity, nface_elem_range, nface_start_offset, ngon_sections, coords_array, device):
    ...
    # Prepare NFACE_n block data (stores cell -> face connectivity)
    if not isinstance(nface_start_offset, wp.array):
        nface_start_offset = wp.array(nface_start_offset, dtype=wp.int32, device=device)

    nface_connectivity_array = wp.array(nface_connectivity, dtype=wp.int32, device=device)

    # Create NFACE_n block using create_dataset
    nface_block_dataset = unstructured.create_dataset(
        grid_coords=coords_array,
        element_type=ET_NFACE_n,
        element_range=wp.vec2i(int(nface_elem_range[0]), int(nface_elem_range[1])),
        element_connectivity=nface_connectivity_array,
        element_start_offset=nface_start_offset,
    )

    # Create NGON_n blocks.
    ngon_dataset_handles = []

    for ngon_data in ngon_sections.values():
        ...
        connectivity_array = wp.array(ngon_data["connectivity"], dtype=wp.int32, device=device)

        # Create NGON_n dataset using create_dataset
        ngon_dataset = unstructured.create_dataset(
            grid_coords=coords_array,
            element_type=ET_NGON_n,
            element_range=wp.vec2i(int(elem_range[0]), int(elem_range[1])),
            element_connectivity=connectivity_array,
            element_start_offset=start_offset,
        )

        ngon_dataset_handles.append(ngon_dataset.handle)

    # Create NFACE_n dataset using create_dataset
    dataset = nface_n.create_dataset(nface_n_block=nface_block_dataset.handle, ngon_n_blocks=ngon_dataset_handles)
...
def read(filename: Union[str, Path], zone: Union[str, None] = None, device: Union[str, wp.context.Device, None] = None) -> CGNSZone:
    ...
    if filepath.suffix.lower() != ".cgns":
        raise ValueError(f"Expected .cgns file extension, got: {filepath.suffix}. If this is a valid CGNS file with a different extension, rename it to use the .cgns extension.")

    # Load the CGNS file using MLL
    try:
        (tree, links, paths) = cgm.load(str(filepath))
    except Exception as e:
        raise ValueError(f"Failed to load CGNS file '{filepath}'. The file may be corrupted or in an unsupported format. Error: {e}") from e
    ...
    try:
        ...
        for elem_section in element_sections:
            ...
            if elem_type < ET_NODE and elem_type >= ET_NofValidElementTypes:
                logger.info("Skipping section '%s' - not a valid element (type=%d)", section_name, elem_type)
                continue
            ...
            if elem_type in [ET_MIXED, ET_NGON_n, ET_NFACE_n]:
                ...
                if element_start_offset is None:
                    if elem_type == ET_MIXED:
                        # MIXED without start offsets - compute them
                        logger.warning("Section '%s' is MIXED but missing ElementStartOffset", section_name)
                        logger.warning("         Start offsets will be computed. Consider adding ElementStartOffset to the CGNS file.")
                    else:
                        # NGON_n and NFACE_n require start offsets
                        elem_type_name = "NGON_n" if elem_type == ET_NGON_n else "NFACE_n"
                        logger.warning("Skipping section '%s' - %s requires ElementStartOffset", section_name, elem_type_name)
                        continue

            # Handle NGON_n sections - store for later association with NFACE_n
            if elem_type == ET_NGON_n:
                ngon_sections[section_name] = {"connectivity": connectivity, "element_range": elem_range, "element_start_offset": element_start_offset}
            ...
            if elem_type == ET_NFACE_n:
                # Create NFACE_n dataset by combining with NGON_n sections
                try:
                    nface_dataset = _create_nface_n_dataset(section_name, connectivity, elem_range, element_start_offset, ngon_sections, coords_array, device)
                    ...
                    logger.exception("Failed to create NFACE_n dataset for '%s': %s", section_name, e)
                    continue

            # Create SIDS unstructured dataset for standard element types (TETRA, HEXA, PYRA, PENTA, MIXED)
            connectivity_array = wp.array(connectivity, dtype=wp.int32, device=device)
            ...
            dataset = unstructured.create_dataset(
                grid_coords=coords_array,
            ...
            logger.info("Loaded section '%s': type=%d, cells=%d, points=%d", section_name, elem_type, elem_range[1] - elem_range[0] + 1, num_points)
        ...
        if flow_fields:
            ...
            for dataset in result.values():
                for field_name, field in flow_fields.items():
                    dataset.add_field(field_name, field)
