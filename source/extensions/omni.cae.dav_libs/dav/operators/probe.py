...
def compute(dataset: dav.DatasetLike, field_name: str, positions: dav.DatasetLike, output_field_name: str = "probed_values") -> dav.DatasetLike:
    """
...
if dav.config.compile_kernels_aot:
    ...
    dataset_data_models = aot.get_data_models(specialization="operators.probe.dataset")
    positions_data_models = aot.get_data_models(specialization="operators.probe.positions")
    field_models = aot.get_field_models(specialization="operators.probe")
    ...
    for data_model in dataset_data_models:
        for field_model in field_models:
            ...
            for positions_data_model in positions_data_models:
                logger.info(f"Compiling kernels for data model: {data_model}, field model: {field_model}, positions data model: {positions_data_model}")
                kernel = get_kernel(data_model, field_model, interpolator, field_model_out, positions_data_model)
