...
@dav.cached(aot="operators.cell_sizes")
def get_kernel(data_model: dav.DataModel):
    @dav.kernel(module="unique")
    @dav.utils.set_qualname("dav_cell_sizes_kernel")
...
if dav.config.compile_kernels_aot:
    ...
    for data_model in aot.get_data_models(specialization="operators.cell_sizes.dataset"):
