...
@dav.cached(aot="operators.point_cell_counts")
def get_kernel(data_model: dav.DataModel):
    @dav.kernel(module="unique")
    @dav.utils.set_qualname("dav_point_cell_counts_kernel")
...
if dav.config.compile_kernels_aot:
    ...
    for data_model in aot.get_data_models(specialization="operators.point_cell_counts.dataset"):
