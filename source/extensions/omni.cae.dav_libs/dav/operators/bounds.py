...
@dav.cached(aot="operators.bounds")
def get_kernel(data_model: dav.DataModel):
    @dav.kernel(module="unique")
    @dav.utils.set_qualname("dav_bounds_kernel")
    def bounds(ds: data_model.DatasetHandle, min_bounds: wp.array(dtype=wp.vec3f), max_bounds: wp.array(dtype=wp.vec3f)):
        point_idx = wp.tid()
...
def compute(dataset: dav.DatasetLike, min_field_name: str = "bounds_min", max_field_name: str = "bounds_max") -> dav.DatasetLike:
    """
    ...
    min_bounds = wp.array([wp.float32(3.4028235e38), wp.float32(3.4028235e38), wp.float32(3.4028235e38)], dtype=wp.vec3f, device=device)
    max_bounds = wp.array([wp.float32(-3.4028235e38), wp.float32(-3.4028235e38), wp.float32(-3.4028235e38)], dtype=wp.vec3f, device=device)
...
if dav.config.compile_kernels_aot:
    ...
    for data_model in aot.get_data_models(specialization="operators.bounds.dataset"):
