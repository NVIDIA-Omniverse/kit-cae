...
@dav.cached(aot="operators.cell_bounds")
def get_kernels(data_model: dav.DataModel):
    @dav.kernel(module="unique")
    @dav.utils.set_qualname("dav_cell_bounds_nb_cells_kernel")
    ...
    @dav.kernel(module="unique")
    @dav.utils.set_qualname("dav_cell_bounds_kernel")
    def cell_bounds_kernel(ds: data_model.DatasetHandle, mins: wp.array(dtype=wp.vec3f), maxs: wp.array(dtype=wp.vec3f)):
        cell_idx = wp.tid()
...
def compute_from_data_model(data_model: dav.DataModel, dataset_handle: dav.DatasetHandle, device) -> tuple[wp.array, wp.array]:
    """
    ...
    """
    kernel, nb_cells_kernel = get_kernels(data_model)
    ...
    mins = wp.zeros((nb_cells,), dtype=wp.vec3f, device=device)
    maxs = wp.zeros((nb_cells,), dtype=wp.vec3f, device=device)
...
def compute(dataset: dav.DatasetLike, min_field_name: str = "cell_bounds_min", max_field_name: str = "cell_bounds_max") -> dav.DatasetLike:
    """
...
if dav.config.compile_kernels_aot:
    ...
    for data_model in aot.get_data_models(specialization="operators.cell_bounds.dataset"):
        logger.info(f"Compiling kernels for data model: {data_model}")
        cell_bounds_kernel, nb_cells_kernel = get_kernels(data_model)
        wp.compile_aot_module(cell_bounds_kernel.module, device=aot.get_devices())
        wp.compile_aot_module(nb_cells_kernel.module, device=aot.get_devices())
