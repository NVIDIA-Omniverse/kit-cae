...
class Options:
    def __init__(self):
        self._schema_types = ["SIDS Unstructured", "Point Cloud"]
        self._current_schema_type = MinimalModal(0, self._schema_types)
        self._allow_pickle = ui.SimpleBoolModel(False)

    def build_options(self, paths: list[str]):
        with ui.VStack(height=0, spacing=4):
            with ui.VStack(height=0, spacing=4):
                with ui.HStack(height=0, spacing=4):
                    ui.Label("Schema:", width=0)
                    combobox = ui.ComboBox(self._current_schema_type)
                    combobox.set_tooltip("Select the schema to use for the dataset.")
                with ui.HStack(height=0, spacing=4):
                    ui.Label("Allow Pickle:", width=0)
                    checkbox = ui.CheckBox(self._allow_pickle)
                    checkbox.set_tooltip("Allow 'pickle' for reading NPY files (only enable for trusted files)")

    @property
    def schema_type(self) -> str:
        return self._current_schema_type.current_text

    @property
    def allow_pickle(self) -> bool:
        return self._allow_pickle.get_value_as_bool()

    @schema_type.setter
    def schema_type(self, value: str):
        self._current_schema_type.current_text = value

    @allow_pickle.setter
    def allow_pickle(self, value: bool):
        self._allow_pickle.set_value(value)


class NPZAssetImporter(AbstractImporterDelegate):
    ...
    async def _convert_asset(self, path: str, import_as_reference: bool, export_folder: str):
        ...
        if import_as_reference:
            ...
            await NPZAssetImporter._populate_stage(stage, path, self.get_options(path))
            stage.Save()
            return usd_path
        else:
    ...
    @staticmethod
    async def _populate_stage(stage: Usd.Stage, path: str, import_options: Options):
        ...
        caeFieldArrayClass = cae.NumPyFieldArray(stage.CreateClassPrim(rootPath.AppendChild("NumPyFieldArrayClass")))
        ...
        if import_options.schema_type == "SIDS Unstructured":
        ...
        elif import_options.schema_type == "Point Cloud":
            ...
            raise RuntimeError("Unsupported schema: %s", import_options.schema_type)
        ...
        coords = [None] * 3
        for name in names:
            fieldArray = cae.NumPyFieldArray.Define(stage, scope.GetPath().AppendChild(Tf.MakeValidIdentifier(name)))
            fieldArray.GetPrim().GetSpecializes().SetSpecializes([caeFieldArrayClass.GetPath()])
            fieldArray.CreateArrayNameAttr().Set(name)

            if relName := NPZAssetImporter._detect_default_relationship(name):
...
async def import_to_stage(
    path: str, prim_path: str, *, schema_type: str = "SIDS Unstructured", allow_pickle: bool = False
) -> Usd.Prim:
    """
    Import a NPZ file into a USD stage at the given prim path.

    Args:
        path: The path to the NPZ file.
        prim_path: The path to the prim to import the NPZ file into.
        schema_type: The schema type to use for the NPZ file.
        allow_pickle: Whether to allow pickle for reading NPY files.
    ...
    if options := importer.get_options(path):
        ...
        options.allow_pickle = allow_pickle
    else:
