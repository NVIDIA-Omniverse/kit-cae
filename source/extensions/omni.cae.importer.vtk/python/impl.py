...
def populate_stage(path: str, local_path: str, stage: Usd.Stage):
    ...
    if output.IsA("vtkImageData"):
        ...
        cae_vtk.ImageDataAPI.Apply(dataset.GetPrim())
        caeImageDataAPI = cae_vtk.ImageDataAPI(dataset)
        caeImageDataAPI.CreateOriginAttr().Set(Gf.Vec3f(output.GetOrigin()))
        caeImageDataAPI.CreateSpacingAttr().Set(Gf.Vec3f(spacing))
        caeImageDataAPI.CreateMinExtentAttr().Set(Gf.Vec3i(wholeExtent[0], wholeExtent[2], wholeExtent[4]))
        caeImageDataAPI.CreateMaxExtentAttr().Set(Gf.Vec3i(wholeExtent[1], wholeExtent[3], wholeExtent[5]))
        ...
        # for polyhedron cells, we also need faces info;
        # since we don't want to actually read the data during import,
        # we create them, no matter what.
        faPrim = _add_special_array(
            "PolyhedronFacesOffsets", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.polyhedron_faces_offsets
        )
        unstructuredGridAPI.CreatePolyhedronFacesOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "PolyhedronFaces", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.polyhedron_faces_connectivity_array
        )
        unstructuredGridAPI.CreatePolyhedronFacesConnectivityArrayRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "PolyhedronFaceLocationsOffsets",
            dataset,
            stage,
            caeFieldArrayClass,
            cae_vtk.Tokens.polyhedron_face_locations_offsets,
        )
        unstructuredGridAPI.CreatePolyhedronFaceLocationsOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "PolyhedronFaceLocations",
            dataset,
            stage,
            caeFieldArrayClass,
            cae_vtk.Tokens.polyhedron_face_locations_connectivity_array,
        )
        unstructuredGridAPI.CreatePolyhedronFaceLocationsConnectivityArrayRel().SetTargets({faPrim.GetPath()})

    elif output.IsA("vtkStructuredGrid"):
        logger.warning("Importing as 'StructuredGrid'")

        outInfo = reader.GetOutputInformation(0)
        spacing = outInfo.Get(vtkDataObject.SPACING())
        wholeExtent = list(outInfo.Get(vtkSDDP.WHOLE_EXTENT()))

        dataset = cae.DataSet.Define(stage, rootPath.AppendChild("VTKStructuredGrid"))
        cae_vtk.StructuredGridAPI.Apply(dataset.GetPrim())
        structuredGridAPI = cae_vtk.StructuredGridAPI(dataset)

        faPrim = _add_special_array("Points", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.points)
        structuredGridAPI.CreatePointsRel().SetTargets({faPrim.GetPath()})
        structuredGridAPI.CreateMinExtentAttr().Set(Gf.Vec3i(wholeExtent[0], wholeExtent[2], wholeExtent[4]))
        structuredGridAPI.CreateMaxExtentAttr().Set(Gf.Vec3i(wholeExtent[1], wholeExtent[3], wholeExtent[5]))
    elif output.IsA("vtkPolyData"):
        logger.info("Importing as 'PolyData'")
        dataset = cae.DataSet.Define(stage, rootPath.AppendChild("VTKPolyData"))
        cae_vtk.PolyDataAPI.Apply(dataset.GetPrim())
        polyDataAPI = cae_vtk.PolyDataAPI(dataset)

        # add special arrays first.
        faPrim = _add_special_array("Points", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.points)
        polyDataAPI.CreatePointsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "VertsConnectivityOffsets", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.verts_connectivity_offsets
        )
        polyDataAPI.CreateVertsConnectivityOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "VertsConnectivity", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.verts_connectivity_array
        )
        polyDataAPI.CreateVertsConnectivityArrayRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "LinesConnectivityOffsets", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.lines_connectivity_offsets
        )
        polyDataAPI.CreateLinesConnectivityOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "LinesConnectivity", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.lines_connectivity_array
        )
        polyDataAPI.CreateLinesConnectivityArrayRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "PolysConnectivityOffsets", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.polys_connectivity_offsets
        )
        polyDataAPI.CreatePolysConnectivityOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "PolysConnectivity", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.polys_connectivity_array
        )
        polyDataAPI.CreatePolysConnectivityArrayRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "StripsConnectivityOffsets", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.strips_connectivity_offsets
        )
        polyDataAPI.CreateStripsConnectivityOffsetsRel().SetTargets({faPrim.GetPath()})

        faPrim = _add_special_array(
            "StripsConnectivity", dataset, stage, caeFieldArrayClass, cae_vtk.Tokens.strips_connectivity_array
        )
        polyDataAPI.CreateStripsConnectivityArrayRel().SetTargets({faPrim.GetPath()})
    else:
