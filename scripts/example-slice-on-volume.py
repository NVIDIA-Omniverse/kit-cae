...
async def main():
    ...
    volume_path: str = "/World/CAE/IndeXVolume_B1_P3"
    omni.kit.commands.execute("CreateCaeIndeXVolume", dataset_path=dataset_path, prim_path=volume_path)

    stage: Usd.Stage = ctx.get_stage()
    volume_prim: Usd.Prim = stage.GetPrimAtPath(volume_path)
    UsdVol.Volume(volume_prim).CreateVisibilityAttr().Set(UsdGeom.Tokens.invisible)

    # 2. Set the field value
    volume_prim.GetRelationship("omni:cae:index:volume:field").SetTargets(
        ["/World/StaticMixer_cgns/Base/StaticMixer/Flow_Solution/Eddy_Viscosity"]
    )

    # 3. Create a slice out the volume use its default colormap and auto-adjusted range
    slice_path: str = "/World/CAE/IndexVolumeSlice_B1_P3"
    omni.kit.commands.execute("CreateCaeIndeXVolumeSlice", dataset_path=volume_path, prim_path=slice_path)
